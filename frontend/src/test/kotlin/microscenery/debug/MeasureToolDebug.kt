...
class MeasureToolDebug() : DefaultScene(VR = true, width = 1024, height = 1024) {
    ...
    override fun init() {
        ...
        thread {
            ...
            (0..4).map { measureTool.apply {
                this.spatial().position = Random.random3DVectorFromRange(-3.0f,3.0f)
                this.spatial().scale = Vector3f(0.3f)
                (this.getAttributeOrNull(Pressable::class.java) as? PerButtonPressable)?.actions?.get(OpenVRHMD.OpenVRButton.Trigger)?.onRelease?.invoke(this.spatial(), TrackedDevice(TrackedDeviceType.Controller,"asd",Matrix4f(),System.nanoTime()))
            } }
